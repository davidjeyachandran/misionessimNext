...
- `scripts/diff-cms-collisions.ts` (built and run 2026-07-04, read-only): compares every exported post against the live Contentful space, computes the per-post `imageVerdict` and revista-link/suspicious-mismatch flags described above, writes `export/collision-diff.json`. Rerun before the real import to catch drift (new WP posts, Contentful edits) since it was last generated.
- `scripts/import-cms.ts` (built 2026-07-04, `yarn import:cms`): consumes `export/collision-diff.json` + `export/posts/*.md` → Contentful Management API writes, via `scripts/lib/markdown-to-richtext.ts` for the body conversion (a hand-rolled converter scoped to exactly the corpus's markdown constructs — paragraphs, h1–h4, bold/italic, links, lists, blockquotes, inline images; 41 unit tests, validated error-free against all 335 real bodies). **Idempotent upserts keyed by slug**; uploads media as assets (whichever side `imageVerdict` picked); on a matching slug WP's title/body overwrite the entry, the winning image per `imageVerdict` is set, and an existing `revista` back-reference is preserved untouched. Existing entries are updated in place — never deleted/recreated.
  - **Two modes.** Default (no flags) is a **dry-run**: computes the full plan and writes `export/import-plan.json`, zero Contentful API calls. `--live --environment=<id>` executes it — requires `CONTENTFUL_MANAGEMENT_TOKEN`, requires an explicit environment (no default), refuses `master` without `--force`.
  - **Dry-run has been run and validated** against the live collision-diff + export data: 120 create, 215 update, 302 WP images to upload, 28 Contentful images kept, 5 with none, 215 revista links preserved — all cross-checked against `diff-cms-collisions.ts`'s independent counts.
  - **The `--live` write path is UNTESTED** — we don't yet have Content Management API credentials scoped to the mi-movilicemos space (`i46buyptg48q`). Method signatures were verified against the SDK's type definitions and pass `tsc --noEmit`, but real execution against even a sandbox environment hasn't happened. Do not run `--live` without first exercising it on a sandbox environment (§5, remaining choice #2).
