...
- `scripts/import-cms.ts` (not yet built): consumes `export/collision-diff.json` + `export/posts/*.md` → Contentful Management API writes. **Idempotent upserts keyed by slug**, batched under rate limits, resumable; uploads media as assets (whichever side `imageVerdict` picked); converts bodies per the §5.1 body-format decision (WP HTML → RichText — the export's markdown is already mojibake/entity-clean) and rewrites body image URLs to asset references. On a matching slug: WP's title/body overwrite the entry, the winning image per `imageVerdict` is set, and an existing `revista` back-reference is preserved, not dropped. Existing entries are updated in place — never deleted/recreated — so mi-movilicemos's other references survive. This is the one script in the pipeline that writes to the shared production space — build and dry-run carefully, on a sandbox environment first (§5, remaining choice #2).
