...
1. Get Management API credentials scoped to `i46buyptg48q` (Contentful web UI → Settings → API keys → Content management tokens) and a **sandbox environment** (not `master`) to dry-run `--live` against for the first time. This is the first real exercise of the untested write path.
